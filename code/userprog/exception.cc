...
void ExceptionHandler(ExceptionType which)
{
  ...
  //Verifiable avec la commande terror
    ...
    if ((which == PageFaultException)) {
      printf("Segfault Error: Memory space is corrupted\n");
      ASSERT(FALSE);
    } else if ((which == ReadOnlyException)) {
      printf("Segfault Error: Read-Only Address used\n");
      interrupt->Halt();
    } else if ((which == BusErrorException)) {
      printf("Segfault Error: Invalid address\n");
      interrupt->Halt();
    } else if ((which == AddressErrorException)) {
      printf("Segfault Error: Address is invalid\n");
      interrupt->Halt();
    } else if ((which == OverflowException)) {
      printf("WARNING: Maximum value was overflowed for process ");
      currentThread->Print();
    } else if ((which == IllegalInstrException)) {
      printf("Instruction is either reserved or inexistent. Halting...\n");
      interrupt->Halt();
    } else if ((which == NoException)) {
      printf("Unexpected Exception error. Halting...\n");
      ASSERT(FALSE);
    } else {
...
{
  int* val;
  machine->ReadMem(userBufferSource, size, val);

  *kernelBufferDestination = *val;
}
...
void SysCallWrite(OpenFileId fileDestination, VirtualAddress userBufferSource, int size){
  if ((fileDestination == ConsoleOutput)) {
    ...
    CopyFromUser(userBufferSource, msg, size);
